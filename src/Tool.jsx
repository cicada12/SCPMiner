...
const Tool = () => {
  ...
  const [dataset, setDataset] = useState("graph_transactional_dataset1.txt");
  const [algorithm, setAlgorithm] = useState("Subgraph Coverage Patterns");
  ...
  const handleRun = () => {
    alert(`Running algorithm with:
- Dataset: ${selectedFile ? selectedFile.name : dataset}
- Algorithm: ${algorithm}
        ...
        <div className="dropdown-group">
          ...
          <select
            ...
            onChange={async (e) => {
              ...
              setDataset(selected);
              setSelectedFile({ name: selected });  // Simulate file

              try {
                const response = await fetch(`/datasets/${selected}`);
                const text = await response.text();
                setFetchedText(text); // Store only here
                setFileText("");      // Clear previous preview
              } catch (err) {
      ...
      {step === 1 && <button className="next-button" onClick={handleNext}>Next</button>}
        ...
        {step >= 2 && (
          <>
            ...
            {step === 2 && <button className="next-button" onClick={handleNext}>Next</button>}
          </>
        )}
        ...
        {step >= 3 && (
          <>
            ...
            <div className="slider-container">
              ...
              <p className="slider-info">Guidelines: Start off with high minimum supports and relative frequency, then gradually reduce them. For maximum overlap start of low then increase gradually.</p>
            </div>
